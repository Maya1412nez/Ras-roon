...
class OverlayImage:
    ...
    def edit_random(self, main_image_width, main_image_height):
        ...
        self.x, self.y = (randint(0, main_image_width - self.width)
                          ), (randint(0, main_image_height- self.height))
        print(self.x, self.y)
    ...
    def get_quality(self):
        ...
        for i in range(len(self.main_matrix)):
            for j in range(len(self.main_matrix[0])):
                if self.main_matrix[i][j] == 0:
                    empty_pixels_qol += 1
        return f'{self.numbers_for_matrix - 1} / {empty_pixels_qol} * 10000 = {((self.numbers_for_matrix - 1) / empty_pixels_qol) * 10000}'
...
def create_small_matrixes():
    quality_list = open('src/rezs/Dirty_table.txt', 'w', encoding='utf-8')
    MATRIX_QUANTITY = 40
    MATRIX_SIZE_COEF = 3
    ...
    for g in range(MATRIX_QUANTITY):
        ...
        QUALITY = choice([9, 10, 11, 12, 13, 14])
        for t in range(QUALITY):
            # all changes. check that everyone is here
            OVER_IMAGE = OverlayImage(NAME)
            OVER_IMAGE.crop_image()
        ...
        MAIN_IMAGE.save_rez(f'Dirty_table/little_matrix_2x2NUM{g}')
